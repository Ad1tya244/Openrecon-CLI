...
const benchmarkEnabled =
  typeof process !== 'undefined' ? !!process.env.TECHNOLOGY_ENGINE_BENCHMARK : false;
...
const TechnologyEngine = {
  ...
  getTechnology: (name) =>
    [
      ...TechnologyEngine.technologies,
      ...TechnologyEngine.requires.map(({ technologies }) => technologies).flat(),
      ...TechnologyEngine.categoryRequires
        .map(({ technologies }) => technologies)
        .flat()
    ].find(({ name: _name }) => name === _name),

  getCategory: (id) => {
    for (const category of TechnologyEngine.categories) {
      if (category.id === id) {
        return category;
      }
    }
  ...
  resolve(detections = []) {
    ...
    TechnologyEngine.resolveExcludes(resolved);
    TechnologyEngine.resolveImplies(resolved);

    const priority = ({ technology: { categories } }) =>
      categories.reduce(
        (max, id) => Math.max(max, TechnologyEngine.getCategory(id).priority),
        0
      );
    ...
    return resolved
      ...
      .map(
        ({
          ...
          description,
          slug,
          categories: categories.map((id) => TechnologyEngine.getCategory(id)),
          confidence,
          version,
          icon,
          website,
  ...
  resolveExcludes(resolved) {
    resolved.forEach(({ technology }) => {
      technology.excludes.forEach(({ name }) => {
        const excluded = TechnologyEngine.getTechnology(name);
      ...
      resolved.forEach(({ technology, confidence, lastUrl }) => {
        technology.implies.forEach(
          ({ name, confidence: _confidence, version }) => {
            const implied = TechnologyEngine.getTechnology(name);
   ...
   * @param {*} param0
   */
  analyze(items, technologies = TechnologyEngine.technologies) {
    benchmarks = [];

    const oo = TechnologyEngine.analyzeOneToOne;
    const om = TechnologyEngine.analyzeOneToMany;
    const mm = TechnologyEngine.analyzeManyToMany;
  ...
  setTechnologies(data) {
    const transform = TechnologyEngine.transformPatterns;

    TechnologyEngine.technologies = Object.keys(data).reduce((technologies, name) => {
      const {
        cats,
        certIssuer,
        cookies,
      ...
      technologies.push({
        ...
        slug: TechnologyEngine.slugify(name),
        text: transform(text),
        url: transform(url),
    ...
    TechnologyEngine.technologies
      ...
      .forEach((technology) =>
        technology.requires.forEach(({ name }) => {
          if (!TechnologyEngine.getTechnology(name)) {
            throw new Error(`Required technology does not exist: ${name}`);
          }

          TechnologyEngine.requires[name] = TechnologyEngine.requires[name] || [];

          TechnologyEngine.requires[name].push(technology);
        })
      );

    TechnologyEngine.requires = Object.keys(TechnologyEngine.requires).map((name) => ({
      name,
      technologies: TechnologyEngine.requires[name]
    }));
    ...
    TechnologyEngine.technologies
      ...
      .forEach((technology) =>
        technology.requiresCategory.forEach(({ id }) => {
          TechnologyEngine.categoryRequires[id] =
            TechnologyEngine.categoryRequires[id] || [];

          TechnologyEngine.categoryRequires[id].push(technology);
        })
      );

    TechnologyEngine.categoryRequires = Object.keys(TechnologyEngine.categoryRequires).map(
      (id) => ({
        categoryId: parseInt(id, 10),
        technologies: TechnologyEngine.categoryRequires[id]
      })
    );

    TechnologyEngine.technologies = TechnologyEngine.technologies.filter(
   ...
   */
  setCategories(data) {
    TechnologyEngine.categories = Object.keys(data)
      .reduce((categories, id) => {
        const category = data[id];

        categories.push({
          id: parseInt(id, 10),
          slug: TechnologyEngine.slugify(category.name),
          ...category
        });
  ...
  transformPatterns(patterns, caseSensitive = false, isRegex = true) {
    ...
    const parsed = Object.keys(patterns).reduce((parsed, key) => {
      parsed[caseSensitive ? key : key.toLowerCase()] = toArray(
        patterns[key]
      ).map((pattern) => TechnologyEngine.parsePattern(pattern, isRegex));
  ...
  parsePattern(pattern, isRegex = true) {
    if (typeof pattern === 'object') {
      return Object.keys(pattern).reduce(
        (parsed, key) => ({
          ...parsed,
          [key]: TechnologyEngine.parsePattern(pattern[key])
        }),
        {}
      );
    } else {
    ...
    return technology[type].reduce((technologies, pattern) => {
      ...
      if (matches) {
        technologies.push({
          ...
          version: TechnologyEngine.resolveVersion(pattern, value)
        });
      }
...
if (typeof module !== 'undefined') {
  module.exports = TechnologyEngine;
}
